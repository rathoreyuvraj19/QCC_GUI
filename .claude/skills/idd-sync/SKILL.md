...
- An LRU slot is `5 * channels_per_lru + 10` bytes (30 for the reference 4-channel array); its checksum is XOR of every preceding byte in the slot (not CRC-8), always the last byte, generated/verified GUI-side, not QCC-side.
...
- **The frame size is configurable, not fixed at 2970.** It's `90 + num_lru * (5 * channels_per_lru + 10)`, from `core/lru_config.py` — 2970 only for the reference 96 LRU x 4-channel array. Read it through `core/packet.py`'s `total_packet_size()` / `num_lru()` / `lru_slot_size()` functions; never snapshot them into a module constant or a default argument (see the invariant in CLAUDE.md). The Remote Programming (`QCC_COMMAND` 0xFF) TX frames are the **exception that stays fixed**, since none of them carries an LRU data block: 90 bytes for the QCC-level speed switches (SubCommand 0x01/0x02 — byte 35 is LRU_SELECT, a 0-based index = one LRU / 0xFF = all of them), 100 bytes for LRU-targeted commands (90 + 10-byte inner bootloader command), and 4196 bytes for bitstream DATA chunks only (90 + 10 + 4096-byte payload). Mode Step 1's per-slot frame is a standard frame and does scale. See `remote_programming_framing` in `packet_spec.yaml`; if your IDD change is about Remote Programming, check `bootloader_packet.py` instead of/in addition to `core/packet.py`.

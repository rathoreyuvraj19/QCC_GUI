...
tx_test_window.py) - lets Yuvraj narrow the per-LRU grid down to just the
LRU(s) he cares about instead of scrolling through every row each time.
...
LruFilterProxyModel:
  ...
  - Auto filter: a QCheckBox (an unambiguous on/off indicator - Yuvraj found
    ...
    that shows only LRUs whose slot isn't all-zero in the *current* frame -
...
def parse_filter_text(text: str):
    """
    Parse a comma-separated list of LRU indices and/or ranges (e.g.
    "2,5,10-15") into a set of ints, clamped to 0..num_lru()-1. Malformed or
    ...
    for token in text.split(","):
        ...
        if "-" in token:
            ...
            if lo > hi:
                lo, hi = hi, lo
            for i in range(max(0, lo), min(num_lru() - 1, hi) + 1):
                rows.add(i)
        else:
            try:
                i = int(token)
...
class LruFilterProxyModel(QSortFilterProxyModel):
...
class FilterBar(QWidget):
    """Manual text filter + auto (non-zero rows) toggle + clear, wired to a LruFilterProxyModel."""

    def __init__(self, proxy_model: LruFilterProxyModel, source_model, parent=None):
        super().__init__(parent)
        ...
        layout.addWidget(QLabel("Filter LRU:"))
